  ...
  return (
    <aside className={`evidence-panel${closing ? " closing" : ""}`} role="dialog" aria-label="Feature to observational evidence">
      ...
      {comparison && state === "comparison" && <>
        ...
        <dl className="match-context"><div><dt>Feature distance</dt><dd>{selectedCandidate?.distance_km} km</dd></div><div><dt>Model grid offset</dt><dd>{comparison.spatial_distance_km} km</dd></div><div><dt>Time matching</dt><dd>{comparison.time_matching.method === "linear_time_interpolation" ? "Linear interpolation" : "Nearest timestep"}</dd></div><div><dt>Common depth</dt><dd>{comparison.depth_matching.common_depth_range.join("–")} m</dd></div><div><dt>Matched levels</dt><dd>{comparison.depth_matching.valid_matched_depths}</dd></div></dl>
        ...
        <details className="scientific-results"><summary>Scientific details</summary><dl className="metric-grid"><div title="Overall difference between paired values; smaller means closer in this scientific context"><dt>RMSE</dt><dd>{comparison.metrics.rmse} {displayUnits(comparison.units)}</dd></div><div title="Average model minus measurement"><dt>Mean bias</dt><dd>{comparison.metrics.bias > 0 ? "+" : ""}{comparison.metrics.bias} {displayUnits(comparison.units)}</dd></div><div title="Average absolute difference"><dt>MAE</dt><dd>{comparison.metrics.mae} {displayUnits(comparison.units)}</dd></div><div title="Depths where both profiles had valid values"><dt>Matched</dt><dd>{comparison.metrics.matched_samples} depths</dd></div></dl><p>RMSE summarizes the overall difference. Smaller values mean the profiles are closer, but scientists interpret it together with the graph and matching context.</p></details>
        <details className="match-method"><summary>How was this matched?</summary><dl><div><dt>Location</dt><dd>Nearest valid wet model cell at {comparison.model_location.lat}°, {comparison.model_location.lon}°</dd></div><div><dt>Time</dt><dd>{comparison.time_matching.previous_model_time} → {comparison.time_matching.next_model_time}; nearest gap {fmtHours(comparison.time_matching.nearest_time_gap_hours)}</dd></div><div><dt>Depth</dt><dd>Model linearly interpolated to valid observation depths</dd></div><div><dt>QC</dt><dd>{comparison.qc_summary.policy}. {comparison.qc_summary.rejected} rejected.</dd></div></dl></details>
      </>}
